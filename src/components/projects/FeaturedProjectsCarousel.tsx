...
export default function FeaturedProjectsCarousel({ dict, lang }: Props) {
  ...
  return (
    <section className="py-14 border-t border-bw-lightgray" aria-labelledby="featured-projects-title">
      ...
      <div
        ref={trackRef}
        className="mt-6 grid auto-cols-[minmax(260px,1fr)] grid-flow-col gap-4 overflow-x-auto snap-x snap-mandatory pb-2"
      >
        {featuredProjects.map((item) => {
          ...
          return (
            <a
              ...
              className="group snap-start overflow-hidden rounded-2xl border border-bw-lightgray bg-white shadow-sm transition motion-safe:duration-300 hover:-translate-y-1 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bw-primary/60"
            >
              <ImageCard
                src={item.image}
                alt={content.title}
                fallbackLabel={dict.home.imageFallback}
                className="h-48 w-full border-b border-bw-lightgray transition motion-safe:duration-300 group-hover:scale-[1.02]"
              />
              <div className="p-4">
